...
class MovimientoInsumoRepository:
    def guardar_lote(self, movimientos: List[MovimientoInsumo]) -> None:
        modelos = [
            MovimientoModel(
                ...
                motivo=m.motivo, usuario_id=m.usuario_id
    ...
    def guardar(self, movimiento: MovimientoInsumo) -> MovimientoInsumo:
        m = MovimientoModel.objects.create(
            ...
            motivo=movimiento.motivo, usuario_id=movimiento.usuario_id
        )
        return MovimientoInsumo(id=m.id, insumo_id=m.insumo_id, tipo=m.tipo, cantidad=m.cantidad, motivo=m.motivo, usuario_id=m.usuario_id, fecha=m.fecha)
